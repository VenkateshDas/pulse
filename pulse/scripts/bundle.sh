...
# The Agent sidecar invokes Pulse's CLI from the app bundle. Build it alongside
# the GUI rather than relying on a developer-installed `pulse` command.
# shellcheck disable=SC2086
swift build -c release --product pulse-cli $ARCH_FLAGS
...
CLI=""
for candidate in ".build/apple/Products/Release/pulse-cli" ".build/release/pulse-cli"; do
    if [ -f "$candidate" ]; then CLI="$candidate"; break; fi
done
[ -n "$CLI" ] || { echo "error: built pulse-cli binary not found"; exit 1; }

rm -rf "$APP"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp "$BIN" "$APP/Contents/MacOS/Pulse"
cp "$CLI" "$APP/Contents/Resources/pulse"
echo "Bundled binary architectures: $(lipo -archs "$APP/Contents/MacOS/Pulse" 2>/dev/null || echo unknown)"

# Copy every SwiftPM resource bundle (e.g. Pulse_Pulse.bundle, generated by
# `resources:` in Package.swift) into Contents/Resources. Without this the
# app crashes on launch: Bundle.module can't find its bundle and fatalErrors.
# The bundles sit alongside the built executable.
BIN_DIR="$(dirname "$BIN")"
shopt -s nullglob
