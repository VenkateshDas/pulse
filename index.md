...
- **Pulse Agent:** `pulse/Sources/Pulse/Views/AgentView.swift` provides the native Agent page; `AgentModel.swift` reduces versioned agent events into a safe timeline. `PulseKit/AgentClient.swift` and `AgentDaemonManager.swift` authenticate the permitted local Agno sidecar. `pulse-agent/` is the sidecar: typed SSE envelopes, bounded context, tool approvals, and SQLite sessions. Design: [Pulse Agent Design](docs/superpowers/specs/2026-09-16-pulse-agent-design.md).
