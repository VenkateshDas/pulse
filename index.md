...
- **Displays (Brightness Control):** `BrightnessEngine.swift` (DDC/DisplayServices, adaptive sync), `MediaKeyManager.swift` (IOHIDManager interception via MediaKeyTap), `SoftwareDimmer.swift` (sub-zero overlay), `DisplaySliderView.swift` (per-monitor slider). Physical brightness keys routed through Pulse for both built-in and external monitors.
